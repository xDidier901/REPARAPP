...
class ReporteEquiposPDF(View):
    ...
    def cabecera(self,pdf):
            ...
            pdf.drawString(230, 790, u"        REPARAPP     ")
            pdf.setFont("Helvetica", 14)
            pdf.drawString(200, 770, u"REPORTE DE EQUIPOS")
            pdf.setFont("Helvetica", 12)
            pdf.drawString( 450, 750, u"Fecha:"+time.strftime("%d/%m/%y"))
    def tabla(self,pdf,y):
            #Creamos una tupla de encabezados para neustra tabla
            encabezados = ('Marca', 'Modelo','Sistema Operativo','Cliente')
            #Creamos una lista de tuplas que van a contener a los Equipos
            detalles = [(equipo.marca, equipo.modelo,equipo.sistema_operativo,equipo.cliente.nombre)for equipo in Equipo.objects.order_by('sistema_operativo','marca')]
            #Establecemos el tamaño de cada una de las columnas de la tabla
            detalle_orden = Table([encabezados] + detalles, colWidths=[3 * cm, 4 * cm, 4 * cm, 5 * cm])
            #Aplicamos estilos a las celdas de la tabla
            detalle_orden.setStyle(TableStyle(
            [
                    #La primera fila(encabezados) va a estar centrada
...
class ReporteClientesPDF(View):
    ...
    def tabla(self,pdf,y):
            #Creamos una tupla de encabezados para neustra tabla
            encabezados = ('Nombre', 'Apellido','Correo','Telefono')
            #Creamos una lista de tuplas que van a contener a los Clientes
            detalles = [(cliente.nombre, cliente.apellido,cliente.correo,cliente.telefono)for cliente in Cliente.objects.order_by('nombre')]
            #Establecemos el tamaño de cada una de las columnas de la tabla
            detalle_orden = Table([encabezados] + detalles, colWidths=[4.5 * cm, 4.3 * cm, 5 * cm, 3.5 * cm])
            #Aplicamos estilos a las celdas de la tabla
            detalle_orden.setStyle(TableStyle(
            [
                    #La primera fila(encabezados) va a estar centrada
